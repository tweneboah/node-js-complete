...
//All task by user
app.get('/tasks2', auth, async (req, res) => {
  try {
    //Option 1
...
// Fitlering
//We will use the concept of query strings
// GET /task?completed=true

app.get('/tasks3', auth, async (req, res) => {
  try {
    //Option 1
    //Find all tasks whose onwer id is equall to the login user
    //You can pass your matches into the find method
    //Return all tasks whose owner id ==== to the loggin user && completed === true
    // const user = await Task.find({ owner: req.user._id, completed: true });
    // res.send(user);
...
// GET /task?completed=true
// GET /tasks?limit=10&skip=0 //First ten results
// GET /tasks?limit=10&skip=10 //second ten results thus second page
//We have to configure mongoose
